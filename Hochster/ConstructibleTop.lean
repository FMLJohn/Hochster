...
lemma instTopologicalSpaceSubtype_eq_generateFrom {X : Type*} [T : TopologicalSpace X]
    {S : Set (Set X)} (hTS : T = generateFrom S) (Y : Set X) :
    instTopologicalSpaceSubtype = generateFrom { s : Set Y | ∃ t ∈ S, s = Y ∩ t } := by
  have : @Set.image _ _ (Set.preimage Subtype.val) ((fun F => ⋂₀ F) '' { F | F.Finite ∧ F ⊆ S })
      = { s : Set Y | ∃ F ⊆ S, F.Finite ∧ s = Y ∩ ⋂₀ F } := by
    ext s
    simp only [Set.mem_image, exists_exists_and_eq_and, Set.preimage_sInter]
    constructor
    · refine fun ⟨F, ⟨hF, hFS⟩, hFs⟩ => ⟨F, hFS, hF, hFs ▸ ?_⟩
      · ext
        simp only [Set.image_val_iInter, Set.mem_iInter, Set.mem_image, Set.mem_preimage,
          Subtype.exists, exists_and_left, exists_prop, exists_eq_right_right]
        exact ⟨fun h => ⟨(h ∅).2, fun t htF => (h t).1 htF⟩, fun ⟨hxY, hFx⟩ t => ⟨hFx t, hxY⟩⟩
    · refine fun ⟨F, hFS, hF, hsF⟩ => ⟨F, ⟨hF, hFS⟩, ?_⟩
      · have : (@Subtype.val X fun x => x ∈ Y) ⁻¹' (Subtype.val '' s) =
            Subtype.val ⁻¹' (Y ∩ ⋂₀ F) :=
          hsF ▸ rfl
        simpa only [Subtype.val_injective, Set.preimage_image_eq, Set.preimage_inter,
          Subtype.coe_preimage_self, Set.preimage_sInter, Set.univ_inter] using this.symm
  refine (this ▸ isTopologicalBasis_subtype (isTopologicalBasis_of_subbasis hTS)
    (fun x => x ∈ Y)).eq_generateFrom ▸ eq_of_le_of_le ?_ ?_
  · refine le_generateFrom ?_
    · simp only [Set.mem_setOf_eq, forall_exists_index, and_imp]
      intro s t htS hst
      exact isOpen_generateFrom_of_mem ⟨{ t }, Set.singleton_subset_iff.mpr htS,
        Set.finite_singleton t, Set.sInter_singleton _ ▸ hst⟩
  · refine le_generateFrom ?_
    · simp only [Set.mem_setOf_eq, forall_exists_index, and_imp]
      intro s F hFS hF hsF
      exact (Set.preimage_sInter ▸ Set.preimage_inter ▸
        Set.preimage_image_eq s Subtype.val_injective ▸ congrArg (Set.preimage Subtype.val) hsF) ▸
          Subtype.coe_preimage_self _ ▸ Set.univ_inter _ ▸
            @Set.Finite.isOpen_biInter _ _ (generateFrom _) _ _ hF
              fun t htF => isOpen_generateFrom_of_mem ⟨t, hFS htF, Subtype.image_preimage_coe Y t⟩


-- lemma instTopologicalSpace_eq (Y : Set X) [SpectralSpace Y] :
