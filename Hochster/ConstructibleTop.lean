...
lemma instTopologicalSpace_eq_generateFrom_isOpen_isCompact
...
lemma instTopologicalSpace_le [CompactSpace X] [QuasiSeparatedSpace X] [PrespectralSpace X] :
...
variable (X) in
instance t2Space_of_spectralSpace [SpectralSpace X] : T2Space (ConstructibleTop X) := by
...
variable (X) in
instance compactSpace_of_spectralSpace [SpectralSpace X] : CompactSpace (ConstructibleTop X) := by
