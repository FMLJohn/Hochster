...
\begin{proof}
  ...
  Now as $O$ is quasi-compact and open, we can find a finite collection $\{D_{a_1},...,D_{a_n}\}$
  of principal open subsets of $Spec(A)$ such that $O=\cup_{i=0}^n{D_{a_i}}$. Then
  \\
  $Spec(h)^{-1}(O)$
  \\
  $=Spec(h)^{-1}(\cup_{i=0}^n{D_{a_i}})$
  \\
  $=\cup_{i=0}^n{Spec(h)^{-1}(D_{a_i})}$
  \\
  $=\cup_{i=0}^n{D_{h(a_i)}}$,
  \\
